...
<!-- 			<div id="slider" class="slider" class="oneImg"> -->
<!-- 				<ul id="slider_touch" class="slide-banner"> -->
<%-- 				<li><a><img alt="" src="${global_image_url}/banner_lxyz.jpg"></a></li> --%>
<!-- 				</ul> -->
<!-- 			</div> -->
<%-- 		</c:otherwise> --%>
<%-- </c:choose> --%>
...
<div class="bszl_list">
	<ul >
		<c:if test="${not empty qsQuestionnaireList}">
       		<c:forEach var="qsQuestionnaire" items="${qsQuestionnaireList}" varStatus="status">
		    	<li>
					<a href="${global_url}/qsQuestionnaire/initQSQuestionnaireInfo.htm?qs_id=${qsQuestionnaire.qs_id }">
		                <span class="bszl_num">${status.index+1}</span>
		                <span class="wj_title">${qsQuestionnaire.title}</span>
		                <p class="wj_p">${qsQuestionnaire.detail}</p>
		           	</a>
				</li>
        	</c:forEach>
	    </c:if>
	    <c:if test="${empty qsQuestionnaireList}">
	    	<li>暂无数据</li>
	    </c:if>
    </ul>
</div>
