...
<script type="text/javascript">
	...
	$(document).bind("scroll", function(event){
 	 ...
 	 function GetAjaxData(){
 		 	var url = "${global_url}/wfw/gzbx/getGZBXListAsync.htm?shequId="+${shequId}+"pageno="+(pageno*1+1)+"&pageSize="+pageSize;
	        $.ajax({
	        	type:"POST",
	 ...
	 function appendHtml(item){
		 var html='<li>'
					+'<div class="rt">'
					+'<div class="tit"><a href="${global_url}/wfw/gzbx/getGZBXInfo.htm?gzbxId='+${item.gzbxId}+'&shequId='+${shequId}+'">'+${item.content }+'<p style="float: right;">'+${item.createdTime }+'</p></a></div>'
					+'</div>'
					+'</li>'
			      ;
		 return html;
...
<div class="grzx_list">
		<ul class="allcon">
			<c:if test="${not empty gzbxList}">
	       	<c:forEach var="gzbxMap" items="${gzbxList}">
	       		<li>
					<a href="${global_url}/wfw/gzbx/getGZBXInfo.htm?gzbxId=${gzbxMap.gzbxId}&shequId=${shequId}">${gzbxMap.content }<p style="float: right;">${gzbxMap.createdTime }</p></a>
		        </li>
	       	</c:forEach>
	       	</c:if>
	    </ul>
    	<c:if test="${empty gzbxList}">
