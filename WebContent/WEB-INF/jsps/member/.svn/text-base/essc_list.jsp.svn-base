...
<script type="text/javascript">
 	 ...
 	 function GetAjaxData(){
 		 	var url = "${global_url}/wfw/essc/getESSCListAsync.htm?pageno="+(pageno*1+1)+"&pageSize="+pageSize;
	        $.ajax({
	        	type:"POST",
	 ...
	 function appendHtml(item){
		 var html='<li>'
					+'<div class="rt">'
					+'<div class="tit"><a href="${global_url}/wsh/essc/getESSCInfo.htm?esscId='+${item.esscId}+'">'+${item.title }+'<p style="float: right;">'+${item.createdTime }+'</p></a></div>'
					+'</div>'
					+'</li>'
			      ;
		 return html;
...
<div class="grzx_list">
		<ul class="allcon">
			<c:if test="${not empty esscList}">
	       	<c:forEach var="esscMap" items="${esscList}">
	       		<li>
					<a href="${global_url}/wsh/essc/getESSCInfo.htm?esscId=${esscMap.esscId}">${esscMap.title }<p style="float: right;">${esscMap.createdTime }</p></a>
		        </li>
	       	</c:forEach>
	       	</c:if>
	    </ul>
    	<c:if test="${empty esscList}">
