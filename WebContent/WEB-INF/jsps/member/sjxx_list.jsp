...
<script type="text/javascript">
 	 ...
 	 function GetAjaxData(){
 		 	var url = "${global_url}/wfw/sjxx/getSJXXListAsync.htm?pageno="+(pageno*1+1)+"&pageSize="+pageSize;
	        $.ajax({
	        	type:"POST",
	 ...
	 function appendHtml(item){
		 var html='<li>'
					+'<div class="rt">'
					+'<div class="tit"><a href="${global_url}/wfw/sjxx/getSJXXInfo.htm?sjxxId='+${item.sjxxId}+'">'+${item.content }+'<p style="float: right;">'+${item.createdTime }+'</p></a></div>'
					+'</div>'
					+'</li>'
			      ;
		 return html;
...
<div class="grzx_list">
		<ul class="allcon">
			<c:if test="${not empty sjxxList}">
	       	<c:forEach var="sjxxMap" items="${sjxxList}">
	       		<li>
					<a href="${global_url}/wfw/sjxx/getSJXXInfo.htm?sjxxId=${sjxxMap.sjxxId}">${sjxxMap.content }<p style="float: right;">${sjxxMap.createdTime }</p></a>
		        </li>
	       	</c:forEach>
	       	</c:if>
	    </ul>
    	<c:if test="${empty sjxxList}">
