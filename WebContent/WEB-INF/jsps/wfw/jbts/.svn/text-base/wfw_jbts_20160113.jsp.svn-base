...
<div class="Courseinfor" style="border-top: 1px #e3e3e3 solid;">
	<div class="course-header" style="border-bottom:none">
        ...
        <div class="course-content">
        	...
        	<img src="" height="50" width="50" id="downloadPic5" style="display:none;">
...
wx.ready(function () {
	...
	document.querySelector('#downloadImage').onclick = function () {
		var serverIdLength = images.serverId.length;
		if (serverIdLength === 0) {
			alert('请先选取图片');
			return;
		}
		...
		function download() {
    		if(serverIdLength>0){
    			for(var i=0;i<serverIdLength;i++){
    				var serverId =  images.serverId[i];
    				//获取微信服务器图片地址
					var result = $.ajax({
					...
					$('#downloadPic'+(i+1)).attr('src',result).show();
